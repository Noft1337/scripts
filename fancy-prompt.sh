...
__powerline() {
    ...
    __git_info() {
        ...
        local GIT_UPDATED_FG="$MICHAEL_FG_LIME"
        local GIT_UPDATED_BG="$MICHAEL_BG_LIME"

        local GIT_CHANGED_FG="$FG_COLOR9"
        ...
        if [ -n "$(git status --porcelain)" ]; then
            printf "%s" "${GIT_CHANGED_BG}${RESET}${GIT_CHANGED_BG} $branch$marks ${GIT_EDIT_EMOJI}  ${GIT_CHANGED_FG}"
        else
            printf "%s" "${GIT_UPDATED_BG}$RESET$FG_BASE02$GIT_UPDATED_BG $branch$marks ${GIT_UPDATED_EMOJI} $RESET$GIT_UPDATED_FG"
        fi
    }
    ...
    ps1() {
        ...
        PS1+="$(__git_info)"
        PS1+="$BG_EXIT$RESET"
        PS1+="$BG_EXIT$FG_BASE3${EMOJI}${RESET}${FG_EXIT}${RESET} "
    }
